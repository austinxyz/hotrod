# dispatch-tracking Specification

## Purpose
Persist per-request dispatch state in the driver service so external events can act on a dispatch after the fact. Introduced by pickup-confirmation: a Redis-backed dispatch record written at dispatch time enables a validated `POST /dispatches/{requestID}/arrived` transition whose notification reaches the rider through the existing frontend polling — the simplest cross-service behavior that unit tests cannot cover, validated end-to-end by a Signadot sandbox plan. Foundation for the future driver-status capability.
## Requirements
